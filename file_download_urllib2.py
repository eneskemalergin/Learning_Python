...
def download(base_url, program_name, topic_name, file_name, file_number, extension = ".mp3"):
	...
	# directory_location =  "C:\Users\Pc-41\Desktop\H.E\ " + program_name + "\ "+ topic_name
        directory_location = "C:\Users\Pc-41\downloads\Minberden Yukselen Ses"
...
"""
number_list = ["%.2d" % i for i in range(16)]
for j in range(len(number_list) - 1):
    string_number = str(number_list[j])
    if string_number == "00":
        continue
    else:
        download('http://www.radyocihan.com/download/mp3/%252Fuploads%252Faudio', 'minberden-yukselen-ses', 'mys-ahlaki-mulahazalar',
    'ahlaki-mulahazalar-izmir-bornova-', string_number, '.mp3')
print "All done!"
